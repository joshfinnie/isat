...
function updateSatrecsPosVel(satrecs, julianDate) {
    ...
    return { satrecs: satrecsOut, positions: positions, velocities: velocities };
}
...
function populateSatelliteBillboard() {
    ...
    for (satnum = 0, max = satData.length; satnum < max; satnum += 1) {
        billboard = satBillboards.add({ imageIndex: 0, position: new Cesium.Cartesian3(0, 0, 0) }); // BOGUS position
...
function showGeolocation(scene) {
    function showGeo(position) {
        var target = ellipsoid.cartographicToCartesian(
            // TODO: should this be 0, 0, 0 through Geolocation?
...
document.getElementById("satellite_button").onclick = function () {
    if (
        document.getElementById("satellite_form").style.display === "none" ||
        !document.getElementById("satellite_form").style.display
    ) {
...
document.getElementById("display_button").onclick = function () {
    if (
        document.getElementById("map_display").style.display === "none" ||
        !document.getElementById("map_display").style.display
    ) {
...
function satelliteHoverDisplay(scene) {
    ...
    handler.setInputAction(
        // actionFunction, mouseEventType, eventModifierKey
        function (movement) {
            var pickedObject = scene.pick(movement.endPosition);
            var satDiv = document.getElementById("satellite_popup");
            var canvasTop = canvas.offsetTop;
            if (pickedObject && pickedObject.satelliteName) {
                // Only show satellite, not Geo marker
...
function displayStats() {
    ...
    document.getElementById("satellite_latInDegrees").innerHTML =
        satrecs[satnum].latInDegrees.toFixed(3);
    document.getElementById("satellite_lonInDegrees").innerHTML =
        satrecs[satnum].lonInDegrees.toFixed(3);
        ...
        function (click) {
            ...
            if (pickedObject) {
                ...
                if (typeof window !== "undefined") {
                    ...
                    if (Number(satDesig.slice(0, 2)) < 20) {
                        // heuristic from JTrack3D source code
                        century = "20";
                    } else {
                        century = "19";
                    }
...
function showOrbit() {
    ...
    orbitTraces.modelMatrix = Cesium.Matrix4.fromRotationTranslation(
        Cesium.Transforms.computeTemeToPseudoFixedMatrix(now),
        Cesium.Cartesian3.ZERO
    );
